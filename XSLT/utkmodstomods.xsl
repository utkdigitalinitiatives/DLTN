<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="pForm">
    ...
    <f authority="aat" uri="http://vocab.getty.edu/aat/300028569">manuscripts (document genre)</f>
  ...
  <xsl:template match="physicalDescription">
    ...
    <xsl:if test="$vForm = $pForm/f">
      <genre authority="{$pForm/f[matches(text(), $vForm)]/@authority}"
        valueURI="{$pForm/f[matches(text(), $vForm)]/@uri}">
  ...
  <xsl:template match="location[not(parent::relatedItem)]">
    <xsl:copy>
      ...
      <xsl:element name="url">
        ...
        <xsl:value-of
          select="concat(following::identifier[starts-with(., 'https://')], '/datastream/TN/view')"
        />
      </xsl:element>
      </xsl:if>
    </xsl:copy>
  ...
  <!-- ignore preceding or following sibling titleInfos when titlInfo[@supplied] is present -->
  <xsl:template
    match="titleInfo[preceding-sibling::titleInfo[@supplied] or following-sibling::titleInfo[@supplied]]"/>
  ...
  <!--
    ...
    -->
  <xsl:template
    match="/mods/recordInfo/recordContentSource[preceding::relatedItem[@type = 'host'][@displayLabel = 'Project']/titleInfo/title[contains(., 'Volunteer Voices')]]">
    <xsl:copy>
  ...
  <xsl:template match="accessCondition[@type = 'local']">
    <abstract> Permission granted for reproduction for use in research and teaching, provided proper
      attribution of source. Credit line should read: [description of item, including photographic
      number], 'Courtesy of McClung Museum of Natural History and Culture, The University of
      Tennessee.' For all other uses consult
      https://mcclungmuseum.utk.edu/research/image-services/rights-reproductions/ or call
      865-974-2144. </abstract>
  </xsl:template>
</xsl:stylesheet>
